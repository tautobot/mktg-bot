...
cd $ROOT; export PYTHONPATH=$PYTHONPATH:$PWD; pipenv sync
  ...
  info=$(pytest -v -s $SCRIPT_HOME/test_self_bought_flep.py 2>&1)
  result=$(echo "$info" | tail -n1 | sed 's/[=]//g')
    if [[ "$result" == *"failed"* ]]; then
      echo "$info" > "$WORKING_DIR_LOGS/self_bought_flep.txt"
  fi
  printf "$result \n";
...
printf '\n=== Flep Sponsor Purchase ===\n'
  ...
  info=$(pytest -v -s $SCRIPT_HOME/test_flep_web_portal.py 2>&1)
  result=$(echo "$info" | tail -n1 | sed 's/[=]//g')
    if [[ "$result" == *"failed"* ]]; then
