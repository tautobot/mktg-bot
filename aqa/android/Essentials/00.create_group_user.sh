...
if [[ "$environment" == 'local' ]]; then
  ...
  $env_postgres "psql -U $user atlas -h localhost  -c 'delete from accounts where user_id in ( select id from users where \"group\"='\'test_aqa_benefits\''); '"  >/dev/null 2>&1
  ...
  $env_atlas "mkdir $WORKING_DIR_FIXTURE" ; $env_atlas "mkdir $WORKING_DIR_LOGS"
